...
def search(request):
    if request.method == "GET":
        ...
        if (len(searched_data)==0):
            ...
            query = (Q(name__icontains=searched_data)| Q(species__icontains=searched_data) | Q(breed__icontains=searched_data) | Q(animal_type__icontains=searched_data))
...
def order_history(request):
    ...
    orders = OrderPet.objects.filter(user=request.user).select_related('order_id','pet').order_by('-order_id__created_at') #-order is ascending/descending
    ...
    order_group = {}
    for order in orders:
        order_number = order.order_id.order_number #will first go and get the order id and then corresponing to the id the order number
