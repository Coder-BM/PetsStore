...
from io import BytesIO #for PDF making
from reportlab.lib.pagesizes import letter #for PDF making(can put A4,A3,etc)
from reportlab.platypus import SimpleDocTemplate,Table,TableStyle,Spacer #for PDF making(making tables in the PDF)
...
def create_order_pdf(order,cart_items):
    ...
    customer_table = Table(customer_info) #to make tables
    customer_table.setStyle(TableStyle([
        ('ALIGN',(0,0),(-1,-1),'LEFT'),  #x and y cordinates will come on the left side
        ('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),
        ('BOOTMPADDING',(0,0),(-1,0),12)
    ]))
...
def send_order_email(order,cart_items):
    ...
    recipient_eamil = order.user.email
    ...
    email = EmailMessage(subject,email_body,from_email,[recipient_eamil])
...
def payments(request):
    context = {}
    if request.method == "POST": #this will work when we get response from payment gateway
        try:
            ...
            for item in cart_items:
                ...
                orderpet.save()
            send_order_email(order_data,cart_items) #calling the send_order_email function          
            cart_items.delete() #removing cart data.
        except json.JSONDecodeError as e:
            return JsonResponse({'error':str(e)})
    else:
        serialized_data = request.GET.get('data') #'data' we are trying to access the url which is written on line 61 redirect_url = f'?data={serialized_data}')
        if serialized_data:
            data = json.loads(serialized_data) #converting the json structure into string
            orders = Orders.objects.get(pk=data['orders']) #data is the variable where 
